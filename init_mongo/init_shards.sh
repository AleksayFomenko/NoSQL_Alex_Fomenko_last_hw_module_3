...
until mongosh --host configsvr:27019 --eval "db.runCommand({ ping: 1 })" >/dev/null 2>&1
do
  sleep 2
done
...
until mongosh --host shard1:27018 --eval "db.runCommand({ ping: 1 })" >/dev/null 2>&1
do
  sleep 2
done
...
sh.shardCollection("university.courses", { course_code: "hashed" })
